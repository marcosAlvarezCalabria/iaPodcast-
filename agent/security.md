...
Audio Pipeline
Capture: capture.worklet.js collects PCM audio from the user's microphone.
Streaming: view-chat.js sends binary PCM data over the WebSocket via GeminiLiveAPI.
Processing: gemini_live.py (backend) receives and forwards audio to Gemini.
Playback: Model responses (PCM audio) are received via WebSocket and played back using playback.worklet.js.
...
ROLEPLAY INSTRUCTION: Define the AI's persona, the user's role, and the scenario.
INTERACTION GUIDELINES: Define behavior constraints (e.g., teaching protocols, proactivity, or strictness).
MISSION COMPLETION: Explicitly define the trigger for the complete_mission tool, including scoring criteria and feedback requirements.
Environment Setup
